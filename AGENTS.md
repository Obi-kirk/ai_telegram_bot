...
- Магазин: /catalog (парсинг data/catalog.txt → 8 товаров по категориям, CatalogService), /cart (add <артикул> [кол-во], clear, checkout; инлайн-кнопки +/−, оформление с подтверждением — без адресов и оплаты, тестовый проект), /status (заказы, статусы pending→Сборка и т.д.), /help. Таблицы cart_items, orders.
